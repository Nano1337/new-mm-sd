...
def resize_image(image, max_size=224):
    """Resize image maintaining aspect ratio with long edge = max_size"""
...
def process_image(image):
    # Add resizing step before processing
...
def main():
    ...
    for i, image in tqdm(enumerate(loader), total=num_samples):
        ...
        # manually resize image to reduce memory footprint
        size = image[0].size
        print(f"Original image size: {size}")
        resized_image = resize_image(image[0])
